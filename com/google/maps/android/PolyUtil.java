...
public class PolyUtil
{
  ...
  public static boolean containsLocation(LatLng paramLatLng, List<LatLng> paramList, boolean paramBoolean)
  {
    ...
    double d1 = Math.toRadians(paramLatLng.latitude);
    double d2 = Math.toRadians(paramLatLng.longitude);
    i = 0;
    paramLatLng = paramList.iterator();
    while (paramLatLng.hasNext())
    {
      paramList = (LatLng)paramLatLng.next();
      double d7 = MathUtil.wrap(d6 - d2, -3.141592653589793D, 3.141592653589793D);
      if ((d5 == d1) && (d7 == 0.0D)) {
        return true;
      }
      double d4 = Math.toRadians(paramList.latitude);
      double d3 = Math.toRadians(paramList.longitude);
      int j = i;
      if (intersects(d1, d4, MathUtil.wrap(d3 - d2, -3.141592653589793D, 3.141592653589793D), d5, d7, paramBoolean)) {
        j = i + 1;
      }
      d1 = d4;
      d2 = d3;
  ...
  public static String encode(List<LatLng> paramList)
  {
    ...
    while (paramList.hasNext())
    {
      ...
      long l3 = Math.round(localLatLng.longitude * 100000.0D);
  ...
  private static boolean isLocationOnEdgeOrPath(LatLng paramLatLng, List<LatLng> paramList, boolean paramBoolean1, boolean paramBoolean2, double paramDouble)
  {
    ...
    if (paramBoolean1)
    {
      ...
      d1 = Math.toRadians(paramLatLng.longitude);
      if (paramBoolean2) {
        paramLatLng = paramList.iterator();
      }
    }
    else
    {
      for (;;)
      {
        ...
        paramDouble = Math.toRadians(paramList.longitude);
    ...
    paramDouble = MathUtil.mercator(d2);
    ...
    while (paramList.hasNext())
    {
      ...
      if ((Math.max(d2, d5) >= d7 - d9) && (Math.min(d2, d5) <= d7 + d9))
      {
        ...
        while (i < j)
        {
          ...
          d1 = d2 * d2 + d12 * d12;
          if (d1 <= 0.0D) {}
          for (d1 = 0.0D; MathUtil.havDistance(d7, MathUtil.inverseMercator(paramDouble + d1 * d12), d11 - d1 * d2) < d6; d1 = MathUtil.clamp((d11 * d2 + (d10 - paramDouble) * d12) / d1, 0.0D, 1.0D)) {
            return true;
          }
          i += 1;
        }
  ...
  private static double sinDeltaBearing(double paramDouble1, double paramDouble2, double paramDouble3, double paramDouble4, double paramDouble5, double paramDouble6)
  {
    ...
    paramDouble5 = Math.sin(paramDouble5 - paramDouble1) + 2.0D * d1 * d3 * MathUtil.hav(paramDouble6);
    paramDouble1 = Math.sin(paramDouble3 - paramDouble1) + 2.0D * d1 * d2 * MathUtil.hav(d4);
...
/* Location:              /Users/michael/Downloads/dex2jar-2.0/GO_JEK.jar!/com/google/maps/android/PolyUtil.class
