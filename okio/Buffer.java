  ...
  public Buffer writeUtf8(String paramString)
  {
    ...
    if (i < k)
    {
      ...
      if (n < 128)
      {
        Segment localSegment = writableSegment(1);
        arrayOfByte = localSegment.data;
        j = localSegment.limit - i;
        int m = Math.min(k, 2048 - j);
        arrayOfByte[(j + i)] = ((byte)n);
        i += 1;
        label91:
        if (i < m)
        {
      ...
      for (;;)
      {
        ...
        {
          writeByte(n >> 6 | 0xC0);
          writeByte(n & 0x3F | 0x80);
          i += 1;
        }
        else if ((n < 55296) || (n > 57343))
        {
          writeByte(n >> 12 | 0xE0);
          writeByte(n >> 6 & 0x3F | 0x80);
          writeByte(n & 0x3F | 0x80);
          i += 1;
        }
        else
        {
          ...
          label332:
          j = 65536 + ((0xFFFF27FF & n) << 10 | 0xFFFF23FF & j);
          writeByte(j >> 18 | 0xF0);
...
/* Location:              /Users/michael/Downloads/dex2jar-2.0/GO_JEK.jar!/okio/Buffer.class
